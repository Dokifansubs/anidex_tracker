...
var returnPeers = function(peer, res) {
    ...
    database.getPeers(peer, function(err, response) {
        if (err) {
            ...
            peer.diff[0] += diff[0];
            peer.diff[1] += diff[1];
            console.log('Response took %d ns', peer.diff[0] * 1e9 + peer.diff[1]);
...
var processPeer = function(err, peer, res) {
    ...
    if (peer.err) {
        console.log('Torrent not in database: ' + peer.info_hash);
        return res.end(common.bencodeFailure(200, 'Torrent not in database.'), 'binary');
    }
