...
var completePeer = function(peer, res) {
    ...
    database.completePeer(peer, function(err, response) {
        if (err) {
            ...
            peer.diff[0] += diff[0];
            peer.diff[1] += diff[1];
            console.log('Peer complete took %d ns', peer.diff[0] * 1e9 + peer.diff[1]);
...
var stopPeer = function(peer, res) {
    if (!peer.canStop()) {
        ...
        return database.getPeersOnly(peer, function(err, response) {
            if (err) {
                ...
                peer.diff[0] += diff[0];
                peer.diff[1] += diff[1];
                console.log('Peers only took %d ns', peer.diff[0] * 1e9 + peer.diff[1]);
                res.end(response, 'binary');
            }
        });
    }
    ...
    database.removePeer(peer, function(err, response) {
        if (err) {
            ...
            peer.diff[0] += diff[0];
            peer.diff[1] += diff[1];
            console.log('Peer stop took %d ns', peer.diff[0] * 1e9 + peer.diff[1]);
            res.end(response, 'binary');
        }
    });
}
...
var updatePeer = function(peer, res) {
    ...
    database.updatePeer(peer, function(err, response) {
        if (err) {
            ...
            peer.diff[0] += diff[0];
            peer.diff[1] += diff[1];
            console.log('Peer update took %d ns', peer.diff[0] * 1e9 + peer.diff[1]);
            res.end(response, 'binary');
        }
    });
}
...
var processPeer = function(err, peer, res) {
    ...
    diff = process.hrtime(peer.queryStart);
    peer.diff[0] += diff[0];
    peer.diff[1] += diff[1];
    peer.queryStart = process.hrtime();
    console.log('Fetched peer %s after %d ns', peer.peer_id , diff[0] * 1e9 + diff[1]);
...
exports.announce = function(req, res) {
    ...
    console.log('Getting peer %s after %d ns', peer.peer_id ,peer.diff[0] * 1e9 + peer.diff[1]);
