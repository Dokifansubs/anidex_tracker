...
exports.hexEncodeIPv6 = function(ip, port) {
    var fields = ip.split(':').length - 1;
    var ipHex = ip.split(':').map(function(value) {
        if (value === '') {
            return hexZerofill('', (8 - fields) * 4);
        }
        return hexZerofill(value, 4);
    });

    // In case people send ::1: addresses
    if (ipHex[0].length > 4) {
        ipHex[0] = '0000';
    }

    if (ipHex[ipHex.length - 1].length > 4) {
        ipHex[ipHex.length - 1] = '0000';
    }
