...
Response.prototype.bencodePeers = function() {
    var peers4 = _.chain(this.peers)
        .filter(function(item) { return item.ipv4 !== null; })
        .map(function(item) {
            var it = {};
...
Response.prototype.bencode = function(peers, peers6) {
    return bencode.encode({
        ...
        'complete': this.item.seeders,
        'incomplete': this.item.leechers,
        'downloaded': this.item.completed,
        'peers': peers,
        'peers6': peers6
    });
};
...
Response.prototype.bencodePeersCompact = function() {
    var string4 = _.chain(this.peers)
        .filter(function(item) { return item.ipv4 !== null; } )
        .map(function(item) { return common.hexEncodeIPv4(item.ipv4, item.port); } )
        .value()
        .join('');
