...
var flushPeers = function() {
    ...
    connection.query('SELECT `id`, `peer_id`, `info_hash`, `left` '
        + 'FROM `peers` '
        + 'WHERE `last_update` < (NOW() - INTERVAL 2 HOUR)'
        , function(err, rows, fields) {
            rows.forEach(function(item) {
                ...
                connection.query('UPDATE `torrent` '
                    + 'SET ' + field + ' = ' + field + ' - 1 '
                    + 'WHERE `info_hash` = ' + connection.escape(item.info_hash) + ' '
                    + 'AND ' + field + ' > 0'
                    , function(err, result) {});
                connection.query('DELETE FROM `peers` '
                    + 'WHERE `id` = ' + connection.escape(item.id)
                    , function(err, result) {
...
exports.removePeer = function(peer) {
    ...
    var field = peer.stored.left == 0 ? 'complete' : 'incomplete';
...
exports.completePeer = function(peer) {
...
var updatePeerInternal = function(peer) {
...
exports.updatePeer = function(peer, callback) {
    // Begin transation.
    connection.beginTransaction(function(err) {
        updatePeerInternal(peer, callback);
    });
};
...
exports.getPeers = function(peer, callback) {
    ...
    var gArguments = [peer.info_hash, peer.numwant];
...
exports.getPeer = function(peer, res, callback) {
    var pQuery = 'SELECT `uploaded`, `downloaded`, `left` '
        + 'FROM `peers` '
        + 'WHERE `id` = ?';

    var tQuery = 'SELECT `info_hash` '
        + 'FROM `torrent` '
        + 'WHERE `info_hash` = ?';

    var pArguments = [peer.id];
    var tArguments = [peer.info_hash];

    connection.beginTransaction(function(err) {
        connection.query(tQuery, tArguments, function(err, rows, fields) {
            if (err) {
                return callback(err, null, null);
            }
            if (rows.length == 0) {
                // TODO: Needs neater error handling.
                callback(1, common.bencodeFailure(200, 'Torrent not in database.'), res);
                return connection.rollback(function() {});
            }
            connection.query(pQuery, pArguments, function(err, rows, fields) {
                if (rows.length == 1) {
